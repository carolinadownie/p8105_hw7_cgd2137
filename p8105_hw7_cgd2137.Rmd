...
sim_df_const_function(50) %>%
  ...
  ggplot(aes(x = theta_hat)) +
  geom_histogram() + labs(
    title = "Distribution of theta_hat when n_sample = 50, bootstrap 5000 times"
  )
```

```{r Problem 4 n_sample is 250}

sim_df_const_function(250) %>%
  bootstrap(n = 5000) %>% 
  mutate(models = map(strap, ~lm(y ~ x, data = .x)), 
         results = map(models, broom::tidy)) %>%
  select(results) %>% 
  ...
  ggplot(aes(x = theta_hat)) +
  geom_histogram() + labs(
    title = "Distribution of theta_hat when n_sample = 250, bootstrap 5000 times"
  )
