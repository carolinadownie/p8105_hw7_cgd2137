...
As this plot shows, the sample averages of β̂2  across tsst for which the null is rejected are not approximately equal to the true value of β2. This makes sense--in order for the null hypothesis that β2=0 to be rejected, we would expect that the estimated β2 is far from zero. 



##Problem 4

```{r Problem 4 setting seed}
...
For various reasons, I’m interested in the distribution of θ̂=log(β̂0β̂1)
. This distribution can be difficult to obtain analytically (although it is not impossible). Using the bootstrap, produce an empirical distribution for θ̂
; illustrate this distribution graphically.
